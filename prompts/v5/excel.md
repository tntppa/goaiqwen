...
你将收到同一票报关资料的 Excel 表格文本。请综合所有 Sheet、区域和上下文进行识别、关联、汇总与校核。

# 要求

1. 若同一字段在不同 Sheet 或区域中出现，优先采用更完整、更清晰、更可信的数据。
2. 若商品明细分散在多个 Sheet 或多个区域中，需合并输出到同一个 `items` 数组。
3. 若发现字段冲突、页间不一致或表格区域不一致，请写入 `error_check_notes`。
